...
package flash.utils {
	...
	*	@since  26.05.2007
	*/
	public class InterpolatedBitmapData {
	   ...
	   */
	   public static function getPixelBicubic(bitmapData:BitmapData, x:Number, y:Number): Number {
	      var i:int = (int)(x);
	      var j:int = (int)(y);

	      if(((i - 1) < 0) || ((j - 1) < 0))
	         return bitmapData.getPixel(i, j);
	      ...
	      for(m = -1; m <= 2; ++m)
	      {
	         for(n = -1; n <= 2; ++n)
	         {
	            var rgb:int = bitmapData.getPixel(i + m, j + n);
	   ...
	   */
		public static function getPixelBilinear(bitmapData:BitmapData, theX:Number, theY:Number): Number {
			...
			if ((x < 1) || (y < 1) || ((x + 2) >= bitmapData.width) || ((y + 2) >= bitmapData.height))
			{
			...
			a = bitmapData.getPixel(x, y);
			be = bitmapData.getPixel(x + 1, y);
			c = bitmapData.getPixel(x, y + 1);
			d = bitmapData.getPixel(x + 1, y + 1);
	   ...
	   */
	   public static function getIntensity (bitmapData:BitmapData, x:int, y:int):int {
	      var rgb:int = bitmapData.getPixel(x, y);
	      var r:int = (rgb >> 16) & 0x0FF;
	      var g:int = (rgb >>  8) & 0x0FF;
	      var b:int = (rgb >>  0) & 0x0FF;
